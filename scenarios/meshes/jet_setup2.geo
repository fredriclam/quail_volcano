SetFactory("Built-in");
//+
// *** Comparison-jet study, with flat wall. ***
...
Field[4] = MathEval;
Field[4].F = Sprintf("((F3/%g)^2 + 1) * %g", a, 0.05*dx);
...
Field[9] = Box;
Field[9].VIn  = 0.05*dx;    // dx in box
Field[9].VOut = 10000*dx;  // dx outside box
Field[9].XMin = 0;
Field[9].XMax = a;
Field[9].YMin = -L;
Field[9].YMax = 0;
...
Physical Curve("x2",    3) = {3};
Physical Curve("symmetry", 4) = {4};
