...
dx = 60;      // Pointwise dx control
// Box dx control
dx_in = 15;    // In-box (region of interest) dx control
dx_out = 45;  // Out-box (region of interest) dx control

r1 = 2400; // Domain size

a = 50;       // Chamber horizontal semi axis
b = 50;       // Chamber horizontal semi axis
c1 = 0.2*a;   // Dike inner x-point (line projects to this number on x-axis)
c2 = c1 + 5;  // Dike outer x-point (line projects to this number on x-axis)
H = 100;      // Depth of center of ellipsoial chamber
R = 5;        // Corner fillet radius generic
R1 = R;       // Corner fillet radius inner
R2 = R;       // Corner fillet radius outer
theta = 69.99*Pi/180; // Dip angle (rad)
