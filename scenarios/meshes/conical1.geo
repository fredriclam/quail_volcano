...
r1 = interior_size*(1-(5/6)^alpha) * 0.5;
// Set conduit radius
a = 50;
// Set corner radius
b = 0.5*a;
...
r2 = interior_size*(1-(4/6)^alpha) * 0.65;
...
Field[2] = MathEval;
Field[2].F = Sprintf("(F1/%g)^2 + %g", 0.5*a, 0.2*dx);

// Set refinement near corner radius
Field[3] = Distance;
Field[3].CurvesList = {43};
Field[4] = MathEval;
Field[4].F = Sprintf("(F3/%g)^2 + %g", b, 0.03*dx);
...
Field[8] = MathEval;
Field[8].F = Sprintf("(F7/200)^2 + %g", dx);
