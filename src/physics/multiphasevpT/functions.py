...
class BCType(Enum):
	...
	MassFluxInlet1D_crystalVFsinusoid = auto()
...
class MassFluxInlet1D_crystalVFsinusoid(BCWeakPrescribed):
	'''
	This class corresponds to an outflow boundary condition with static
	incoming mass flux, for a fixed chamber/reservoir pressure and temperature.
	The boundary state is computed using acoustic Riemann invariants in 1D.
	Varies crystal volume fraction sinusoidally in time while maintaining
	constant dissolved water content (ASSUMES NO EXSOLVED GAS AT BOUNDARY)
	Attributes:
	-----------
	mass_flux:        mass flux at boundary, const
	p_chamber:        pressure of chamber, const (not necessarily bdry value)
	...
	def get_boundary_state(self, physics, UqI, normals, x, t):
		...
		# Update adiabatically compressed/expanded tracer partial densities
		UqB[:,:,5:] *= rho / atomics.rho(arhoVecI)
		phi_crys = 0.4025 * (1.1 - 0.1 * np.cos(2 * np.pi * 0.0277778 * t)) # crystal vol / suspension vol
		chi_water = 0.05055
		UqB[:,:,5] = rho * chi_water * (1 - phi_crys) / (1 + chi_water)
		UqB[:,:,6] = rho * phi_crys
